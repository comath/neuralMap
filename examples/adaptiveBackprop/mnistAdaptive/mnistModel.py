...
inDim = 28*28
startHiddenDim = currentHiddenDim = 15
maxHiddenDim = 75
outDim = 10
batchSize = 300
...
for itter in range(10):
	...
	for hd in range(currentHiddenDim,maxHiddenDim+1):
		with tf.name_scope('NeuralNetwork%(hidDim)ditter%(itter)d'%{'hidDim':hd,'itter':itter}):
			...
			softmaxLoss = tf.summary.scalar('softmax_loss%(hidDim)ditter%(itter)d'%{'hidDim':hd,'itter':itter}, crossEntropy)
			...
			tf.summary.scalar('accuracy%(hidDim)ditter%(itter)d'%{'hidDim':hd,'itter':itter}, accuracy[hd])
	...
	sess = tf.Session()
	writer = tf.summary.FileWriter('tfLogs/adaptiveStart%(hidDim)dEnd%(maxDim)d'%{'hidDim':startHiddenDim,'maxDim':maxHiddenDim}, sess.graph)
	init = tf.global_variables_initializer()
	sess.run(init)


	for i in range((maxHiddenDim-startHiddenDim + 2)*1000):
...
for itter in range(10):
	...
	sess = tf.Session()
	writer = tf.summary.FileWriter('tfLogs/normalstart%(hidDim)dEnd%(maxDim)d'%{'hidDim':startHiddenDim,'maxDim':maxHiddenDim}, sess.graph)
	init = tf.global_variables_initializer()
	sess.run(init)
	...
	for i in range((maxHiddenDim-startHiddenDim + 2)*1000):
		tr_x, tr_y = mnist.train.next_batch(batchSize)
		feedDict = {X:tr_x, Y:tr_y}
		summ,lossVal,trainVal = sess.run([merged,loss,trainOp], feed_dict = feedDict)
		writer.add_summary(summ, i)